  ...
  var serverMethodCaller= function()
  {
    ...
    for (var i = 0; i < arguments.length; i++){
      ...
      if (typeof arguments[i] == "function"){
    	  callbackFunctionIndex=i;
  	  }
    }
    if(callbackFunctionIndex>-1){
    	while (true){
        	if (typeof args[0] == "function"){
        		callback = args.shift();
        		break;
        	}else{
        		callbackArgs.push(args.shift());
        	}
        }
    }
    var req = JSONRpcClient._makeRequest(this, methodName, args, this.objectID,callback,callbackArgs);
    if (!callback){
...
JSONRpcClient._async_handler = function ()
{
  ...
  while (JSONRpcClient.async_responses.length > 0)
  {
    ...
    {
    	var callbackArgs=[];
    	callbackArgs.push(res.result);
    	callbackArgs.push(res.ex);
    	callbackArgs.push(res.profile);
        ...
        res.cb.apply(this,callbackArgs);  
    }
    catch(e)
    {
...
JSONRpcClient._makeRequest = function (client,methodName, args,objectID,cb,cbargs)
{
  var req = {};
  req.client = client;
  ...
  if (req.cb)
  {
    http.onreadystatechange = function()
    {
      ...
      if (http.readyState == 4)
      {
        ...
        res = {cb: req.cb,cbargs:req.cbargs,result: null, ex: null};
        if (req.profile)
        {
...
*/
  ...
  if (status != 200)
  {
	  var errorObject={ code: status, message: statusText };
	  handlerException(errorObject)
  };
